...
"""  
    Pseudocode for loading truck method
        ...
        put id 19 package and all packages with time deadline in truck 1
...
def load_trucks():
    ...
    for x in h.table:
        for i in x:
            # get all packages and keep them in the packages list
            packages.append(i)

    for line in h.table:
        for p in line:
            # load all delay, wrong and end of day packages to truck 3
            if (p.deadline == 'EOD' and 'Delayed' in p.note) or 'Wrong' in p.note:
                truck3.insert(p)
                packages.remove(p)

            # load all 10:30 deadline and delayed packages to the truck 2
            elif 'Can only' in p.note or ('10:30' in p.deadline and 'Delayed' in p.note):
                truck2.insert(p)
                packages.remove(p)

            # load 9:00, 10:30 deadlind packages to the truck 1 including package id 19.
            elif p.id == 19 or '10:30' in p.deadline or '9:00' in p.deadline:
                truck1.insert(p)
                packages.remove(p)

    # add more packages if each truck is less than 16 packages
    for pk in packages:
...
def check_status(h, m, s):
    ...
    # Process for truck 1.
    truck1.route = get_route(truck1)
    truck1.start_time(2021, 1, 1, 8, 00, 00)
    set_status(truck1)
    print('\nTruck 1 delivery \nTruck 1 started at 8:00 AM.\n')
    delivery(truck1, h, m, s)
    distance1 = get_mileages(truck1)
    print(f'Truck 1 travels {distance1:.2f} miles.')
    truck_returning_time = truck1.get_time()

    # Process for truck 2.
    truck2.route = get_route(truck2)
    truck2.start_time(2021, 1, 1, 9, 10, 00)
    set_status(truck2)
    print('\n\n\nTruck 2 delivery \nTruck 2 started at 9:10 AM.\n')
    delivery(truck2, h, m, s)
    distance2 = get_mileages(truck2)
    ...
    if truck_returning_time > search_time:
        ...
        printing_packages(truck3)
        print('Truck 3 is still at Hub at this time.')
    else:
        fix_address()
        truck3.route = get_route(truck3)
        truck3.time = truck1.get_time()
        t3_time = truck3.time.strftime('%H:%M:%S %p')
        set_status(truck3)
        print(f'\n\n\nTruck 3 delivery \nTruck 3 started at {t3_time}.\n')
        delivery(truck3, h, m, s)
        distance3 = get_mileages(truck3)
